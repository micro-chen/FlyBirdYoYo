...
namespace FlyBirdYoYo.Utilities.Caching
{
    ...
    public class RedisCacheManager : ICacheManager
    {
        ...
        public RedisCacheManager() : this(RedisConfig.LoadConfig())
        {
        }
        /// <summary>
        ...
        /// <returns></returns>
        public ScanResult Scan(string pattern, int pageSize = 10, long cursor = 0)
        {
            try
            {
                int db = Database.Database;
                var result = _connectionWrapper.Scan(db, pattern, pageSize, cursor);
