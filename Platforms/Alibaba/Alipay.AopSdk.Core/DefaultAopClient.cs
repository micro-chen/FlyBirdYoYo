...
namespace Alipay.AopSdk.Core
{
	...
	public class DefaultAopClient : IAopClient
	{
		...
		public T PageExecute<T>(IAopRequest<T> request, string accessToken, string reqMethod) where T : AopResponse,new()
		{
		...
		public T SdkExecute<T>(IAopRequest<T> request) where T : AopResponse,new()
		{
            ...
            // 构造结果
            var rsp = new T();//(T) Activator.CreateInstance(typeof(T));
		...
		private static ResponseParseItem parseRespItem<T>(IAopRequest<T> request, string respBody, IAopParser<T> parser,
			string encryptKey, string encryptType, string charset) where T : AopResponse,new()
		{
		...
		public static void CheckResponseSign<T>(IAopRequest<T> request, string responseBody, bool isError,
			IAopParser<T> parser, string alipayPublicKey, string charset, string signType) where T : AopResponse,new()
		{
		...
		/// <returns></returns>
		private AopDictionary SerializeBizModel<T>(AopDictionary requestParams, IAopRequest<T> request) where T : AopResponse,new()
		{
